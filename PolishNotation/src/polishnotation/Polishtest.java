...
class Polishtest {
    ...
    public Polishtest(String inputDeger) {
        this.inputDeger = inputDeger;
    }
    
    
    static boolean operatör(String harf) {
    ...
    static String güncelle(String input, int result, String oldDeger) {
        
//Bir artimetik işlem yapıldığında sonuc yazılarak en son hali yazılıyor
    ...
    static int PrefixHesap(String input) {
        input += " ";
        ...
        while (geciciInputIndis < strGeciciInput.length()) {
            ...
            if (operatör(singleTekKarakter)) {
                ...
                if (geciciBosluk == -1 || !operatör(tempSingleTekKarakter) || !opcodeTemp.isEmpty()) {
                    ...
                    if (!katarIslem[0].isEmpty() && indexKatarIslem == 1) { //2 tane opcode varsa(1 2)
                        ...
                        strGeciciInput = güncelle(strGeciciInput, result, islemeKatilan);
